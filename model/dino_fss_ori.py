...
def showhet(feature, id):
    ...
    fea = feature[0]
    if len(fea.shape) != 3:
        fea = rearrange(fea, 'C A B D E-> C (A B) (D E)', A=30, B=30, D=30, E=30)
    channel_activations = fea.mean(axis=(1, 2)).cpu()

    # 找到平均激活值最大的通道索引
    max_activation_channel_index = np.argmax(channel_activations)

    # 选择激活值最大的通道
    max_activation_channel = fea[max_activation_channel_index]

    # 归一化选定的通道
    channel_min, channel_max = max_activation_channel.min(), max_activation_channel.max()
    normalized_channel = (max_activation_channel - channel_min) / (channel_max - channel_min)
    normalized_channel = normalized_channel.cpu()
    # 将特征图转换为热力图
    heatmap = plt.get_cmap('jet')(normalized_channel)
...
class HCCNet(nn.Module):
    ...
    def forward(self, query_img, support_img, support_mask):
        with torch.no_grad():
            ...
            # showhet(query_feats[11], self.stack_ids)
    ...
    def predict_mask_nshot(self, batch, nshot):
        ...
        for s_idx in range(nshot):
            ...
            # macs, params = profile(self, inputs=(batch['query_img'], batch['support_imgs'][:, s_idx], batch['support_masks'][:, s_idx]), verbose=False)
