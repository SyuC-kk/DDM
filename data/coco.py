...
class DatasetCOCO(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        query_img, query_mask, support_imgs, support_masks, query_name, support_names, class_sample, org_qry_imsize, q_deg, s_deg = self.load_frame()
    ...
    def load_frame(self):
        ...
        c_query_name = os.path.join(query_name.split('/')[0]+"/random/", query_name.split('/')[1])
        q_deg_path = os.path.join(self.base_path, c_query_name.split('.')[0] + '.txt')
        with open(q_deg_path, "r", encoding="utf-8") as file:
            q_degtype = file.read()
        query_img = Image.open(os.path.join(self.base_path, c_query_name)).convert('RGB')
        ...
        while True:  # keep sampling support set if query == support
            ...
            c_support_name = os.path.join(support_name.split('/')[0] + "/random/", support_name.split('/')[1])
            s_deg_path = os.path.join(self.base_path, c_support_name.split('.')[0] + '.txt')
            with open(s_deg_path, "r", encoding="utf-8") as file:
                s_degtype = file.read()
            if query_name != support_name:
                support_names.append(support_name)
                c_support_names.append(c_support_name)
                s_degtypes.append(s_degtype)
            if len(support_names) == self.shot: break

        support_imgs = []
        support_masks = []
        for support_name, c_support_name in zip(support_names,c_support_names):
            support_imgs.append(Image.open(os.path.join(self.base_path, c_support_name)).convert('RGB'))
        ...
        return query_img, query_mask, support_imgs, support_masks, query_name, support_names, class_sample, org_qry_imsize, q_degtype, s_degtypes
