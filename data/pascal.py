...
class DatasetPASCAL(Dataset):
    def __init__(self, datapath, fold, transform, split, shot, use_original_imgsize):
        ...
        self.img_path = os.path.join(datapath, 'VOC2012/Degradation/random/')
    ...
    def __getitem__(self, idx):
        idx %= len(self.img_metadata)  # for testing, as n_images < 1000 [0,len(img_metadata-1]
        query_name, support_names, class_sample = self.sample_episode(idx) #class_sample is id
        query_img, query_cmask, support_imgs, support_cmasks, org_qry_imsize, q_deg, s_deg = self.load_frame(query_name, support_names)
    ...
    def load_frame(self, query_name, support_names):
        query_img, q_deg_type = self.read_img(query_name) #deg_type
        query_mask = self.read_mask(query_name)
        support_imgs, s_deg_types = zip(*[self.read_img(name) for name in support_names])
        support_masks = [self.read_mask(name) for name in support_names]

        org_qry_imsize = query_img.size

        return query_img, query_mask, support_imgs, support_masks, org_qry_imsize, q_deg_type, s_deg_types
    ...
    def read_img(self, img_name):
        ...
        img = Image.open(os.path.join(self.img_path, img_name) + '.jpg')
        file_path = os.path.join(self.img_path, img_name) + '.txt'
    ...
    def build_img_metadata(self):
        novel = [self.fold * 5 + 1 + v for v in range(5)]
        ...
        def read_metadata(split, fold_id):
            ...
            out = []
            if self.split == 'trn':
                # Image.open(os.path.join(self.ann_path, img_name) + '.png')
                for x in range(len(fold_n_metadata)):
                    i = np.array(Image.open(os.path.join(self.ann_path, fold_n_metadata[x][0]) + '.png'))
                    label_class = np.unique(i).tolist()
                    if 0 in label_class:
                    ...
                    for c in label_class:
                        if c in novel:
                            b = False
                    if b:
                        out.append([fold_n_metadata[x][0], fold_n_metadata[x][1]])
                return out
