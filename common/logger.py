...
class AverageMeter:
    ...
    def write_process(self, batch_idx, datalen, epoch, write_batch_idx=20):
        if batch_idx % write_batch_idx == 0:
            ...
            iou, fb_iou = self.compute_iou()
            # output_file = "DDM_fold3_results_seed345.txt"
            # with open(output_file, "a") as f:
            #     f.write(f"iou: {iou} \t fb_iou: {fb_iou}\n")
            if epoch != -1:
                loss_buf = torch.stack(self.loss_buf)
...
class Logger:
    ...
    def initialize(cls, args, training):
        ...
        logpath = args.logpath if training else args.logpath+ '/' + args.load.split('/')[-3] + '_' + args.load.split('/')[-2].split('.')[0]# + logtime
        if logpath == '': logpath = logtime

        cls.logpath = os.path.join(logpath)# + '.log')
    ...
    #     torch.save(state_dict, os.path.join(cls.logpath, 'train_1_epoch_' + str(epoch) + '_{:.4f}'.format(val_miou) + '.pth'))
    #     cls.info('Model saved @%d w/ val. mIoU: %5.2f.\n' % (epoch, val_miou))

    def save_model_miou(cls, model, shot, epoch, val_miou, optimizer):
        # torch.save(model.state_dict(), os.path.join(cls.logpath, 'best_model.pt'))
        state_dict = {"net": model.state_dict(), "optimizer": optimizer.state_dict(), "epoch": epoch}
        # torch.save(state_dict, os.path.join(cls.logpath, 'best_model.pt'))
        torch.save(state_dict, os.path.join(cls.logpath, 'train_' + str(shot) + '_epoch_' + str(epoch) + '_{:.4f}'.format(val_miou) + '.pth'))
        cls.info('Model saved @%d w/ val. mIoU: %5.2f.\n' % (epoch, val_miou))
