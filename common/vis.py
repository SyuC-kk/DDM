...
class Visualizer:
    ...
    def visualize_prediction(cls, spt_imgs, spt_masks, qry_img, qry_mask, pred_mask, cls_id, batch_idx,
        ...
        merged_pil = cls.merge_image_pair([spport_image, query_image] + [qry_masked_pil, pred_masked_pil])
        ...
        query_name = query_name.split('/')[-1].split('.')[0]
        support_name = support_name[0].split('/')[-1].split('.')[0]
        # merged_pil.save(cls.vis_path + '%d_query_%s_supp_%s_%d_class-%d_iou-%.2f' % (
        # batch_idx, query_name, support_name, sample_idx, cls_id, iou) + '.jpg')  # 1-shot coco
        merged_pil.save(cls.vis_path + '%d_query_%s_support_%s_%d_class-%d_iou-%.2f' % (batch_idx, query_name, support_name, sample_idx, cls_id, iou) + '.jpg')  # 1-shot pascal
